...
void *runner(void *param)
{
    ...
    // bool last = ptr->last;
    struct node* temp=*(ptr->numbers);
    while (temp!=NULL) {
        if (check_perfect(temp->number)) {
            fprintf(*(ptr->file), "%d: Is a perfect number\n", temp->number);
            Insert(ptr->head, temp->number);
        }
        else {
...
int main(int argc, char **argv)
{
    ...
    while (fscanf(input, "%d %d", &n, &k) != EOF)
    {
        ...
        struct node** nums;
        nums=(struct node**)malloc(sizeof(struct node*)*k);
        for (int i=0;i<k;i++) {
            int count=0;
            if (i!=k-1) {
                while (count<max_nums) {
                    if (count%2==0) {
                        Insert(&(nums[i]),num_from_start);
                        num_from_start++;
                    }
                    else {
                        Insert(&(nums[i]),num_from_end);
                        num_from_end--;
                    }
                    count++;
                }
        ...
        for (int i = 0; i < k; i++) {
            freeList(&arrayOfPerfectNumbers[i]);
            freeList(&nums[i]);
        }
        free(files);
        free(nums);
