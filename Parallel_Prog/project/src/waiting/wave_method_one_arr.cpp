...
void *calculate_new_values(void* thread_id) {
    ...
    int iter, diagonal_number, ip , jp, i, j, counter;
    ...
    for (iter = 0; iter < max_iter; iter++) {
        ...
        for (counter = thrId; counter <= nx*ny; counter += num_threads) {
            ip = (counter + ny - 1) / ny;
            jp = counter - (ip - 1)*ny;
...
int main()
{
    ...
    pthread_barrier_init(&bar_upd2, NULL, num_threads+1);
    ...
    pthread_barrier_destroy(&bar_upd2);
