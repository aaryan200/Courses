...
void calculate_new_values(int thread_id) {
    ...
    for (iter = 0; iter < max_iter; iter++) {
        cout << "Iteration " << iter+1 << " started by thread " << thread_id << endl;
        // Calculate new values
        diagonal_number = thread_id;
        while (diagonal_number <= num_diagonals) {
            ...
            while (ip >= 1 and jp <= ny) {
                ...
                // cout << "Point " << ip << "," << jp << " updated by thread" << thread_id << endl;
        ...
        cout << "Thread " << thread_id << " done with phase 1 iteration " << iter+1 << endl;

        calculate_error = true;

        barrier_updation.arrive_and_wait();
        cout << "Thread " << thread_id << " done with phase 2 iteration " << iter+1 << endl;

        if (rel_err < tol) break;
    }
    cout << ">>> Thread " << thread_id << " done after " << min(iter+1, max_iter) << " iterations" << endl;
}
...
void update_values() {
    ...
    for (iter = 0; iter < max_iter; iter++) {
        while (!calculate_error) {
            // Wait for all threads to calculate new values
        }
...
long long solve_gssor_wave()
{
    ...
    // Initialize calculate_error to false
    calculate_error = false;
    ...
    for (i=1; i <= num_threads; i++) {
        calculate_new_values_threads.push_back(thread(calculate_new_values, i));
    }
