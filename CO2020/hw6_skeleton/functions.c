...
void output_soln(int nx, int it, double tcurr, double *x, double *rho, double *u, double *p, double *e)
{
  ...
  sprintf(fname, "primitives_x_%04d.dat", it);
  //printf("\n%s\n", fname);

  fp = fopen(fname, "w");
  for(i=0; i<nx; i++)
    fprintf(fp, "%.6e %.6e %.6e %.6e %.6e\n", x[i], rho[i], u[i], p[i], e[i]);
  fclose(fp);
...
void timestep_LaxWendroff(int nx, int nvars, double dt, double dx, double *x, 
                 double *rho, double *u, double *p, double *e, double **uvec, double **flux, double **umid, double CFL)
{
  int i, j;
  double dtdxfac;
  FILE* fp;
  ...
  for(i=1; i<nx-1; i++)
    for(j=0; j<nvars; j++)
    {
        umid[i][j] = -dtdxfac*(flux[i+1][j] - flux[i][j]);
        umid[i][j] += 0.5*(uvec[i+1][j] + uvec[i][j]);
    }

  // corrector step
  
  // -- step 1: compute primitives corresponding to umid in Lax wanderoff
  dtdxfac = dt/dx;
  // -- step 2: compute flux corresponding to umid
  // get_flux_from_primitves(nx, x, rho, umid, p, e, flux);
  for (i=0; i<nx-1; i++)
    for (j=0; j<nvars; j++)
      umid[i][j] = umid[i][j]*CFL;

  // -- step 3: then update uvec
  for(i=1; i<nx-1; i++)
    for(j=0; j<nvars; j++) {
      uvec[i][j] = umid[i][j] - dtdxfac*(umid[i][j] - umid[i-1][j]);
      // uvec[i][j] = umid[i][j];
    }
