...
int main(int argc, char **argv)
{
    ...
    if (fscanf(input, "%d %d", &k, &N) != EOF)
    {
        ...
        for (int i = 0; i < k; i++)
        {
            ...
            arguments[i].temp_sudoku = (int**)malloc(N*sizeof(int*));
            for (int j=0;j<N;j++) arguments[i].temp_sudoku[j] = (int*)malloc(N*sizeof(int));
            for (int row = 0;row<N;row++) {
                for (int col = 0;col<N;col++) arguments[i].temp_sudoku[row][col] = sudoku[row][col];
            }
            arguments[i].N = N;
